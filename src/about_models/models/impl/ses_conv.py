...
class SESConv_Z2_H(nn.Module):
    '''Scale Equivariant Steerable Convolution: Z2 -> (S x Z2) # S -> scales
    [B, C, H, W] -> [B, C', S, H', W']
    ...
    Args:
        ...
        effective_size: The effective size of the kernel with the same # (number) of params
    ...
    def __init__(self, in_channels, out_channels, kernel_size, effective_size, scales=[1.0], stride=1, padding=0, bias=False, basis_type='A', **kwargs):
        ...
        # Tranformation of the filter in basesFilters V x V -> Nb x S x V x V
        if basis_type == 'A':
